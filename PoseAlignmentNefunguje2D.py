...
def IsTransformExtreme(transform):
    ...
    if abs(abs(transform[0,0]) - transform[1,1]) > 0.3:
        return True
    # keep shear low
    if abs(transform[1,0] + transform[0,1]) > 0.5:
        return True
    # keep rotation low
    if abs(transform[1,0]) > 0.5 or abs(transform[0,1]) > 0.5:
        return True
...
def calculateAffineTansformation(p1, p2, eps):
    ...
    for cycle in range(0, 1000):
        ...
        # calculate model
        homography = cv2.estimateAffinePartial2D(PossibleInliers[:, [0, 1]], PossibleInliers[:, [3, 4]])
...
def getRansacAffineTransformation(p1, p2, eps=50):
    ...
    stackedPoints = np.hstack((p1, p2))
    filtered = np.float32(stackedPoints[np.all(stackedPoints != 0, axis=1)])
    transformation, rigid_mask = cv2.estimateAffinePartial2D(filtered[:, [0, 1]], filtered[:, [3, 4]])
    transformation = np.vstack((transformation, [0, 0, 1]))

    stackedPoints = np.hstack((p1Mirrored, p2))
    filtered = np.float32(stackedPoints[np.all(stackedPoints != 0, axis=1)])
    mirrored_transformation, mirrored_rigid_mask = cv2.estimateAffinePartial2D(filtered[:, [0, 1]], filtered[:, [3, 4]])
    mirrored_transformation = np.vstack((mirrored_transformation, [0, 0, 1]))

    if len(mirrored_rigid_mask) > len(rigid_mask):
        info = {
            "InliersCount": len(mirrored_rigid_mask),
        }
        return None, info
